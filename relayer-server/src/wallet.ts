...
import ERC20 from "../../eth/dist/contracts/IERC20.sol/IERC20.json";
...
export class RelayerWallet {
  ...
  public static async fromAddress(
    ...
    return new RelayerWallet(signer, provider);
  }
  ...
  async topUpFromDonor(
    ...
    const donorWallet = await RelayerWallet.fromAddress(donor, this.provider);
    const donorBalance = await donorWallet.tokenBalance(token);
    if (donorBalance < amount) {
      throw new Error(
        `Donor ${donor} has insufficient balance for token ${token}. Required: ${amount}, Available: ${donorBalance}`
      );
    }
